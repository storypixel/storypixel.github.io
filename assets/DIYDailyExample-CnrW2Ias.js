...
import{u as v,b as k,j as E,N as C}from"./vendor-BepC_yDj.js";import{f as A,a as L}from"./diyDailyData-BcI1kRKb.js";import{_ as M}from"./three-BlfmmWNd.js";async function y(o){if(navigator.clipboard&&window.isSecureContext){await navigator.clipboard.writeText(o);return}const t=document.createElement("textarea");t.value=o,t.style.position="fixed",t.style.opacity="0",document.body.appendChild(t),t.select(),document.execCommand("copy"),document.body.removeChild(t)}function P(o,t){const e=URL.createObjectURL(o),n=document.createElement("a");n.href=e,n.download=t,document.body.appendChild(n),n.click(),document.body.removeChild(n),setTimeout(()=>URL.revokeObjectURL(e),1500)}async function U(o){const{default:t}=await M(async()=>{const{default:n}=await import("./jszip.min-lOPZzDci.js").then(r=>r.j);return{default:n}},__vite__mapDeps([0,1])),e=new t;return e.file("index.html",o),e.generateAsync({type:"blob"})}function S(o){return new URL(o,window.location.origin).toString().split("?")[0]}function D(){const{exampleId:o}=v(),t=A(o);return k.useEffect(()=>{if(!t)return;let e=!1;const n=L(t.id);return fetch(n).then(r=>{if(!r.ok)throw new Error(`Failed to load example: ${r.status}`);return r.text()}).then(r=>{if(e)return;const s=new DOMParser().parseFromString(r,"text/html").documentElement;if(!s)return;const l=document.adoptNode(s);document.replaceChild(l,document.documentElement),z({exampleId:t.id,html:r,sourceUrl:n})}).catch(()=>{e||window.location.replace(n)}),()=>{e=!0}},[t]),t?null:E.jsx(C,{to:"/clients/diy-daily",replace:!0})}function z({exampleId:o,html:t,sourceUrl:e}){if(document.getElementById("diy-export-root"))return;const n=S(e),r=document.createElement("style");r.id="diy-export-style",r.textContent=`
    #diy-export-root {
      position: fixed;
      right: 18px;
      bottom: 18px;
